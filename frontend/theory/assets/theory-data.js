// Shared lesson data for the standalone static theory site.
window.THEORY_DAYS = [
    {
      day: 1,
      icon: 'fa-compass',
    ...
    },
    {
      ...
      goal: '분산투자, 상관관계, 변동성, MDD, 샤프 비율과 파생상품 위험관리·외국인 수급 화면을 읽는 기본을 설명할 수 있어요.',
      keywords: ['포지션', '포트폴리오', '헤지', '변동성 매매', '외국인 수급', '미결제약정', '증거금', '마진콜', '상관관계', '변동성', 'MDD', '샤프 비율', '위험기여도', '집중위험'],
      lessons: [
        ...
        ['HTS에서 외국인 KOSPI 200 선물·옵션 수급 보기', ['HTS 검색창에서 “투자자별 매매동향”, “투자자별 거래동향”, “투자주체”, “수급”을 찾은 뒤 국내 파생상품 시장을 선택합니다. 표에서 외국인 열과 KOSPI 200 선물·콜옵션·풋옵션 행을 각각 확인하세요. 증권사마다 화면번호와 메뉴명은 달라도 이런 구조가 공통적입니다.', '외국인 열의 매수·매도·순매수와 조회 조건을 함께 읽습니다. 당일인지 기간 누적인지, 정규장인지 야간장인지, 계약 수인지 금액인지가 다르면 숫자를 직접 비교할 수 없습니다. 표준 선물과 미니 선물도 거래승수가 달라 계약 수만 합산하면 안 됩니다.', '예를 들어 외국인의 KOSPI 200 선물 당일 순매수가 +3,000계약이면, 그 조회 기준에서 매수가 매도보다 3,000계약 많았다는 뜻입니다. 이것만으로 지수 상승을 확정할 수는 없습니다. 현물·ETF 헤지, 차익거래, 월물 교체가 섞일 수 있기 때문입니다.', '옵션은 특히 콜·풋을 분리해 보고, 거래량·순매수와 미결제약정(OI)을 구분합니다. 콜이나 풋 거래가 늘었다고 해서 외국인이 그 옵션을 매수해 한 방향에 베팅했다고 단정할 수는 없습니다. 옵션 가격과 OI 변화, 기초지수 움직임을 함께 확인합니다. 장 마감 뒤에는 KRX의 파생상품 투자자별 거래실적으로 교차검증하세요.'], { src: 'https://securities.koreainvestment.com/pro_help/images/5336_01.gif', alt: '한국투자증권 HTS의 투자자별 매매동향 예시 화면. 외국인, 개인, 기관 열과 정규시장 선물, 콜옵션, 풋옵션 행이 보인다.', caption: '한국투자증권 HTS 도움말의 예시 화면입니다. 과거 화면으로 현재 시세나 매매 신호가 아니며, HTS별 메뉴·표시 단위는 다를 수 있습니다.', sourceHref: 'https://securities.koreainvestment.com/pro_help/5336.html' }],
        ...
        ['선물포인트와 거래승수로 손익 계산하기', ['선물포인트는 선물 가격 또는 지수가 움직인 크기입니다. 예를 들어 340.00에서 341.20으로 오르면 +1.20포인트입니다. 거래승수는 이 포인트를 실제 원화 금액으로 바꾸는 숫자입니다.', '계약금액은 “선물가격 × 거래승수”, 1계약 손익은 “가격변화 × 거래승수”로 계산합니다. 가령 거래승수가 250,000원인 KOSPI 200 선물이 340.00포인트라면 1계약 규모는 8,500만 원입니다. 1계약을 매수한 뒤 1.20포인트 오르면 수수료 등을 제외한 손익은 +30만 원이고, 1.20포인트 내리면 -30만 원입니다.', '같은 1.20포인트 변화라도 거래승수가 50,000원인 미니 계약이면 손익은 6만 원입니다. 매도 포지션은 방향이 반대여서 가격이 내려갈 때 이익, 오를 때 손실이 납니다. 최소호가단위도 꼭 확인하세요. 예를 들어 최소호가가 0.05포인트이고 승수가 250,000원이라면 한 호가의 금액은 12,500원입니다.', '거래승수·최소호가단위·증거금은 상품과 시장 상황에 따라 다를 수 있습니다. 예시는 계산 연습용이므로 실제 거래에서는 거래소 상품명세와 증권사 주문 화면의 계약 단위를 기준으로 판단해야 합니다.']],
  ...
  const GLOSSARY = [
    ...
    { terms: ['선물포인트', '포인트'], korean: '선물포인트', hanja: '—', abbr: 'pt', english: 'Point', summary: '선물 가격 또는 기초지수가 움직인 크기를 나타내는 단위입니다.', detail: '포인트는 그 자체로 원화 금액이 아닙니다. 예를 들어 340.00에서 341.20으로 움직이면 +1.20포인트이고, 실제 손익은 이 변화에 거래승수와 계약 수를 곱해 계산합니다.' },
    { terms: ['거래승수'], korean: '거래승수', hanja: '去來乘數', abbr: 'Multiplier', english: 'Contract Multiplier', summary: '선물 1계약의 포인트를 실제 금액으로 환산할 때 곱하는 숫자입니다.', detail: '계약금액은 선물가격 × 거래승수, 1계약 손익은 가격변화 × 거래승수로 계산합니다. 예를 들어 승수가 250,000원이면 1.20포인트 변화는 300,000원의 손익입니다. 승수가 클수록 같은 지수 변동에도 손익 금액이 커집니다.' },
