(function () {
  ...
  if (day === 1 || day === 4) {
    ...
    addAfterText(target('젠포트·퀀터스 같은 노코드 퀀트 플랫폼'), `<p class="lesson-video-cta platform-url-links"><button type="button" class="breaktime-trigger" id="noCodeQuantBreakTimeTrigger"><i class="fa-solid fa-mug-hot"></i> BreakTime · 직접 사용해 보기</button><a class="lesson-video-link" href="https://genport.newsystock.com/Main.aspx" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-arrow-up-right-from-square"></i> 젠포트 · genport.newsystock.com</a><a class="lesson-video-link" href="https://www.quantus.kr/ko" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-arrow-up-right-from-square"></i> 퀀터스 · quantus.kr</a><small>서비스 화면·기능·이용 조건은 각 공식 홈페이지에서 확인하세요.</small></p>`);
    if (!document.getElementById('noCodeQuantBreakTimeModal')) {
      document.body.insertAdjacentHTML('beforeend', `<div class="glossary-modal breaktime-modal" id="noCodeQuantBreakTimeModal" hidden><div class="glossary-modal__backdrop" data-no-code-quant-close></div><section class="glossary-modal__dialog breaktime-dialog" role="dialog" aria-modal="true" aria-labelledby="noCodeQuantBreakTimeTitle"><button class="glossary-modal__close" type="button" aria-label="닫기" data-no-code-quant-close>×</button><p class="glossary-modal__label">BREAKTIME · 직접 살펴보기</p><h2 id="noCodeQuantBreakTimeTitle">노코드 퀀트 플랫폼을 직접 사용해 보세요</h2><p>공식 사이트에서 전략 만들기와 백테스트 화면을 직접 살펴보세요. 회원가입, 무료 이용 범위, 계좌 연동과 이용 요금은 각 서비스의 현재 안내를 확인해야 합니다.</p><p class="lesson-video-cta"><a class="lesson-video-link" href="https://genport.newsystock.com/Main.aspx" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-arrow-up-right-from-square"></i> 젠포트 방문하기</a><a class="lesson-video-link" href="https://www.quantus.kr/ko" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-arrow-up-right-from-square"></i> 퀀터스 방문하기</a></p><p><small>처음에는 실제 주문 연결 대신, 조건 설정 → 백테스트 → 기간·비용을 바꾼 재검증 순서로 기능을 익혀 보세요.</small></p></section></div>`);
    }
    const noCodeQuantTrigger = document.getElementById('noCodeQuantBreakTimeTrigger');
    const noCodeQuantModal = document.getElementById('noCodeQuantBreakTimeModal');
    if (noCodeQuantTrigger && noCodeQuantModal && !noCodeQuantTrigger.dataset.ready) {
      const closeNoCodeQuant = () => { noCodeQuantModal.hidden = true; noCodeQuantTrigger.focus(); };
      noCodeQuantTrigger.addEventListener('click', () => { noCodeQuantModal.hidden = false; noCodeQuantModal.querySelector('.glossary-modal__close')?.focus(); });
      noCodeQuantModal.querySelectorAll('[data-no-code-quant-close]').forEach((element) => element.addEventListener('click', closeNoCodeQuant));
      noCodeQuantTrigger.dataset.ready = 'true';
    }
    ...
    addAfterText(target('국내 3개 페어로 보는 과거 신호'), `<p class="lesson-video-cta"><button type="button" class="breaktime-trigger" id="koreanPairBreakTimeTrigger"><i class="fa-solid fa-mug-hot"></i> BreakTime · 비교 종목 넓혀 보기</button><small>페어 정보를 더 찾아, 다른 종목 조합도 비교할 수 있도록 시스템을 확장해 보세요.</small></p>`);
